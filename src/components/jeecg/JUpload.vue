    ...
    methods:{
      initFileListArr(val){
        ...
        for(var a=0;a<val.length;a++){
          if(val[a] && val[a].filePath && val[a].fileName){
      ...
      initFileList(paths){
        if(!paths || paths.length==0 || paths === 'null' || paths === 'undefined'){
          this.fileList = [];
          return;
        }
        // 确保paths是字符串类型
        paths = String(paths);
        let fileList = [];
        let arr = paths.split(",").filter(item => item && item.trim() !== '');
