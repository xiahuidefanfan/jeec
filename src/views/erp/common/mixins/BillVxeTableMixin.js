...
export const BillVxeTableMixin = {
  ...
  methods: {
    ...
    setMaterialUnitOptions(row, entryJVxeTable) {
      ...
      let material = null
      if (row.materialId && row.materialId.length > 0) debugger
    ...
    handleMaterialChange(row, entryJVxeTable, emptyColumnKeys) {
      ...
      if (!!col && !col.disabled)
        col.options =
          material && material.validUnitList ? material.validUnitList.map((u) => ({ value: u.id, text: u.name })) : []
    ...
    requestSubDatas(entryTable, srcTable, srcEntryTable, success, srcBillType, srcBillIds) {
      let that = this
      if (entryTable) that.requestSubTableData(entryTable.url.list, { id: this.model.id }, entryTable, success1)
      else {
      ...
      function success2() {
        ...
        else typeof success === 'function' ? success() : ''
      }
    },
    ...
    removeFromSrcTable(jxSrcTable, srcTable, srcEntryTable) {
      ...
      //id被引用的记录不能移除
      let ids2 = []
      for (let id of ids) if (!dstRows.find((r) => r.srcBillId === id)) ids2.push(id)

      //留下主表记录未删除的
      if (ids2.length > 0 && srcEntryTable) {
        let ds = []
        for (let r of srcEntryTable.dataSource) if (!ids2.includes(r.mid)) ds.push(r)
        srcEntryTable.dataSource = ds
      }
      ...
      if (ids2.length > 0) {
        let ds = []
        for (let r of srcTable.dataSource) if (!ids2.includes(r.id)) ds.push(r)
        srcTable.dataSource = ds
      }
