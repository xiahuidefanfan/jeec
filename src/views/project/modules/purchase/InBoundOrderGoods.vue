<template>
  ...
  >
    <template slot="footer">
      ...
      <a-button v-if="!disableSubmit && action === 'add'" key="submit" @click="inboundApply" type="primary">到货入库</a-button>
      <a-button v-if="!disableSubmit && action === 'return'" key="submit" @click="inboundApply" type="primary">退货</a-button>
    </template>
...
export default {
  ...
  methods: {
    async inboundApply() {
      ...
      this.$refs.realForm.submit(this.action);
    },
    handleSave() {
