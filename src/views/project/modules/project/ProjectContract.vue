    ...
    <div class="project-module-form">
      <div class="project-module-form-header">
        <div class="project-module-form-title">
          <span v-if="!model.contractNo">销售合同 - 新增</span>
          <span v-else>销售合同 - {{ model.contractNo }}</span>
          <a-button
            ...
            <a-row>
              ...
              <a-col :span="8">
                <a-form-model-item
                  ...
                  required
                >
                  <span slot="label">签订日期<span style="color: red">*</span></span>
                  <j-date placeholder="请选择" v-model="model.signDate" class="date-input" :disabled="disabled">
                  </j-date>
                </a-form-model-item>
              </a-col>
              <a-col :span="8">
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        ...
        getContractNo: '/project/contract/getContractNo',
  ...
  methods: {
    beforeUpload(file) {
      if(this.model.contractNo) {
        return new Promise((resolve) => {
          const extension = file.name.slice(file.name.lastIndexOf('.'))
          const fileName = this.model.contractNo + '_销售合同原件'
        ...
        return new Promise((resolve) => {
          getAction(this.url.getContractNo).then((res) => {
            this.model.contractNo = res.result
            const extension = file.name.slice(file.name.lastIndexOf('.'))
            const fileName = this.model.contractNo + '_销售合同原件'
            const newName = `${fileName}${extension}`
            const modifiedFile = new File([file], newName, { type: file.type })
